	...
	<view class="listPage">
		...
		<view class="history">
			<view class="operate">
				...
				<view class="right">
					<fc-select placeholder="品种" type="search" :options='brandList' :value='search.productId'
						@change='handleCategoryChange' allowClear />
					<fc-picker placeholder="时间" type="search" mode="time" @change='handleTimeChange'
						:params='timeParams' :value='timeVal' allowClear />
				</view>
			</view>
			...
			<view class="list-content">
				<fc-list @scrollLower='scrollLower' :count="total" :page="pageNum" :limit='pageSize' :key='tabIndex'
					...
					<view class="panel" v-for="item in list" :key="item.id">
						...
						<fc-detail class="border" title="详情信息" :renderList="renderList('detail',item)"
							@activeChange='(value)=>handleActiveChange(value,item.id)'></fc-detail>
					</view>
				</fc-list>
			</view>
		</view>
